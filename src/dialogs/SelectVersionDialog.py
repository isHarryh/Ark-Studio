...
class SelectVersionDialog(ctk.CTkToplevel):
    def __init__(self, master: App):
        ...
        # Components
        self.title_label = ctk.CTkLabel(self, text="选择资源版本号", image=icon("goto"), **style("panel_title"))
        ...
        self.confirm_button = uic.OperationButton(
            self.button_group, 0, 0, "确认", image=icon("dialog_okay"), command=self._on_confirm
        )

        self.cancel_button = uic.OperationButton(
            self.button_group, 0, 1, "取消", image=icon("dialog_cancel"), command=self.destroy
        )
    ...
    def _fetch_next_page_versions_from_agd(self):
        ...
        self._agd_p += 1
        commits = client.get_commits(self._agd_repo, sha=self._agd_branch, per_page=self._agd_pp, page=self._agd_p)
        rst: "list[acp.ArkVersion]" = []
        for c in commits:
            m = re.match(r"\[([A-Z]{2}) UPDATE\] Client:([\d\.]+) Data:([a-zA-Z\d\-_]+)", c.message)
