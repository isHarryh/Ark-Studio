...
class ResourceManagerPage(ctk.CTkFrame, uic.HidableGridWidget):
    ...
    def invoke_inspect(self, info:ac.FileInfoBase):
    ...
    def invoke_load_tree(self, repo:ac.AssetRepoBase):
        self.repo = repo
...
class _ResourceReloadTask(GUITaskBase):
    ...
    def _run(self):
        ...
        if self._manager.local_root:
            repo = ac.ArkLocalAssetsRepo(self._manager.local_root)
...
class _ResourceSwitchLatestTask(GUITaskBase):
    ...
    def _run(self):
        self._manager.abstract.set_loading(True)
        if isinstance(self._manager.repo, (ac.ArkIntegratedAssetRepo, ac.ArkLocalAssetsRepo)):
            self.update(0.1, "正在获取网路配置")
            config = ac.ArkNetworkConfig(requests.get(ac.ArkNetworkConfig.SOURCE, timeout=9).json())
            self.update(0.3, "正在查询最新版本")
            version = ac.ArkVersion(requests.get(config.api_version(), timeout=9).json())
            self.update(0.5, "正在获取资源列表")
            remote = ac.ArkRemoteAssetsRepo(requests.get(f"{config.api_assets(version.res)}/hot_update_list.json", timeout=9).json())
            self.update(0.8, "正在加载浏览视图")
            if isinstance(self._manager.repo, ac.ArkIntegratedAssetRepo):
                self._manager.repo = ac.ArkIntegratedAssetRepo(self._manager.repo.local, remote)
            else:
                self._manager.repo = ac.ArkIntegratedAssetRepo(self._manager.repo, remote)
            self.update(0.9)
...
class _AbstractPanel(ctk.CTkFrame):
    ...
    def show_repo_res_version(self, repo:ac.AssetRepoBase):
        self.info_local_ver.show("<未知>")
        self.info_remote_ver.show("<未知>")
        local_ver = ""
        if isinstance(repo, ac.ArkLocalAssetsRepo):
            local_ver = repo.detect_res_version()
        elif isinstance(repo, ac.ArkIntegratedAssetRepo):
...
class _ExplorerPanel(ctk.CTkFrame):
    ...
    def __init__(self, master:ResourceManagerPage):
        ...
        self.treeview.set_value_extractor(lambda x:(ac.FileStatus.to_str(x.status), x.get_file_size_str()))
    ...
    def load_tree(self, repo:ac.AssetRepoBase):
        with TestRT('repo_load_tree'):
            ...
            if repo is not None:
                ...
                self.treeview.insert(list(self.children_map[ac.DirFileInfo('')]))
...
class _InspectorPanel(ctk.CTkFrame):
    ...
    def inspect(self, info:ac.FileInfoBase):
        self.info_name.set_body_text(info.basename, "<空>")
        self.info_location.show(info.parent.name, "<根>")
        if isinstance(info, ac.ArkIntegratedFileInfo):
            self.info_status.show(ac.FileStatus.to_str(info.status), "<未知>")
        ...
        elif isinstance(info, ac.ArkLocalFileInfo):
        ...
        elif isinstance(info, ac.DirFileInfo):
...
class _OperationPanel(ctk.CTkFrame):
    ...
    def inspect(self, info:ac.FileInfoBase):
        if isinstance(info, (ac.ArkIntegratedFileInfo, ac.ArkLocalFileInfo, ac.DirFileInfo)):
            if not isinstance(info, ac.DirFileInfo) and \
                info.status in (ac.FileStatus.ADD, ac.FileStatus.DELETE, ac.FileStatus.MODIFY):
            ...
            if not isinstance(info, ac.DirFileInfo) and os.path.exists(info.path):
