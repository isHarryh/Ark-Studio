...
class _ExplorerPanel(ctk.CTkFrame):
    ...
    def __init__(self, master: ResourceManagerPage):
        ...
        self.treeview: "uic.TreeviewFrame[acp.FileInfoBase]" = uic.TreeviewFrame(
            self, 1, 0, columns=3, empty_tip="列表为空"
        )
...
class _OperationPanel(ctk.CTkFrame):
    ...
    def __init__(self, master: ResourceManagerPage):
        ...
        self.btn_view = uic.OperationButton(self, 1, 0, "查看此文件", image=icon("file_view"))
        self.btn_sync = uic.OperationButton(
            self,
            2,
            0,
        ...
        self.btn_goto = uic.OperationButton(
            self, 3, 0, "在文件夹中显示", image=icon("file_goto"), **style("operation_button_info")
        )

    def inspect(self, info: acp.FileInfoBase):
        if isinstance(info, (acp.ArkIntegratedFileInfo, acp.ArkLocalFileInfo, acp.DirFileInfo)):
            if not isinstance(info, acp.DirFileInfo) and info.status in (
                acp.FileStatus.ADD,
                acp.FileStatus.DELETE,
                acp.FileStatus.MODIFY,
            ):
...
class _ResourceSyncFileTask(GUITaskBase):
    ...
    def _run(self):
        if isinstance(self._manager.repo, acp.ArkIntegratedAssetRepo) and isinstance(
            self._info, acp.ArkIntegratedFileInfo
        ):
