...
class _DefFont:
    ...
    def __init__(
        ...
        self._data = _KwDict(
            family=family, size=size, weight="bold" if bold else "normal", slant="italic" if italic else "roman"
        )
...
class _DefImage:
    ...
    def __init__(self, path: str, size: "tuple[int,int]|int", repaint: str = None, use_ctk: bool = True):
        self._data = _KwDict(
            path=path, size=size if isinstance(size, tuple) else (size, size), repaint=repaint, use_ctk=use_ctk
        )
        self._image = None
...
class _StyleHub:
    ...
    DATA: "dict[str,dict]" = {
        ...
        "menu_btn": {"font": FONT_LB, "width": 150, "height": 35},
        "menu_btn_regular": {"fg_color": THEME[0], "hover_color": THEME[1], "text_color": THEME[9]},
        "menu_btn_active": {"fg_color": THEME[5], "hover_color": THEME[6], "text_color": THEME[0]},
        # Panel
        "panel_title": {"font": FONT_MB, "text_color": THEME[3], "anchor": "w", "compound": "left"},
        "panel_title_grid": {"padx": 10, "pady": 10, "sticky": "nw"},
        # Button
        "operation_button": {"font": FONT_S},
        "operation_button_info": {
            "fg_color": (THEME[1], THEME[8]),
            "hover_color": (THEME[2], THEME[7]),
            "text_color": (THEME[7], THEME[2]),
            "border_color": (THEME[5], THEME[5]),
            "border_width": 1,
        },
        "operation_button_grid": {"padx": (5, 10), "pady": (5, 10)},
        # Label
        "info_label_head": {
            "font": FONT_MB,
            "corner_radius": 10,
        ...
        "info_label_body": {"font": FONT_S, "anchor": "nw"},
        "info_label_head_grid": {"padx": 5, "pady": 0, "sticky": "nw"},
        "info_label_body_grid": {"padx": (10, 5), "pady": 0, "sticky": "nw"},
        "info_label_grid": {"padx": 5, "pady": 5, "sticky": "ew"},
        # Progress bar
        "progress_bar_head": {"font": FONT_M, "text_color": THEME[5], "anchor": "w"},
        "progress_bar_body": {"font": FONT_S, "text_color": (THEME[3], THEME[6]), "anchor": "e"},
        "progress_bar_head_grid": {"padx": 5, "pady": 0, "sticky": "nw"},
        "progress_bar_body_grid": {"padx": 5, "pady": 0, "sticky": "ne"},
        "progress_bar_grid": {"padx": 10, "pady": 5},
        # Treeview
        "treeview_empty_tip": {"font": FONT_L},
        # Audio Controller
        "audio_ctrl_name": {"font": FONT_M, "anchor": "center"},
        "audio_ctrl_info": {"font": FONT_S, "anchor": "center"},
        "audio_ctrl_operation_grid": {"padx": 10, "pady": (0, 5)},
    }
